  ...
  if (reg_line){
    slope <- function(x, y) {
      ...
      if (!is.na(ratio) & ratio > 0){
        ...
        # return the evaluation of the ratio condition as third value in numeric vector to control downstream processing
        return(c(round(reg$b0,2), round(reg$b1,2), !is.na(ratio) & ratio > 0))
      }
      # if ratio condition is not met then assign NA to vector so that NULL condition does not throw error below
    ...
    plot1 <- plot1 +
      geom_abline(data = filter(sub_data, row_number() == 1), # only need to return 1 row within group_by
